...
createProtobufLoader(Loader &loader, llvm::ArrayRef<TypeRef> inputImageType) {
  // The image name that the model expects must be passed on the command line.
  std::vector<const char *> inputNames;
  std::vector<std::string> inputNamesVec;
  loader.getModelInputs(inputNamesVec);
  for (const auto &name : inputNamesVec) {
  ...
  if (c2Model) {
    LD.reset(new Caffe2ModelLoader(
        ...
        inputNames, inputImageType, *loader.getFunction()));
  } else {
    LD.reset(new ONNXModelLoader(loader.getOnnxModelFilename(), inputNames,
...
buildAndCompileAndGetInAndOutPair(Loader &loader, PlaceholderBindings &bindings,
  ...
  auto LD = createProtobufLoader(loader, types);

  // get all the input placeholders.
  llvm::StringMap<Placeholder *> inMap = LD->getInputVarsMapping();
  CHECK_EQ(inMap.size(), modelInputsOpt.size())
      << "Number of model inputs must match provided number of inputs.";

  std::vector<std::string> inputNames;
  loader.getModelInputs(inputNames);
  for (auto &name : inputNames) {
    auto it = inMap.find(name);
    CHECK(it != inMap.end()) << "Not an input: " << name;
  }
  // get all the outputs.
  llvm::StringMap<Placeholder *> outMap = LD->getOutputVarsMapping();
  loader.postModelLoad(bindings, *LD.get(), outMap, types);
  ...
  loader.compile(cctx);
  return std::make_pair(inMap, outMap);
}
